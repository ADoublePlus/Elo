...
namespace Elo
{
	...
	class Player
	{
		...
		public:
			...
			std::vector<Match> get_matches() { return matches; }
	...
	// x successes in n trials using the Wilson score interval (see: https://arxiv.org/pdf/1303.1288.pdf).
	...
	IntervalEstimate estimate_rating_difference(int wins, int draws, int losses, double p = 0.95)
	{
		if (wins < 0 || draws < 0 || losses < 0)
			throw std::invalid_argument("wins, draws, and losses must be non-negative.");

		// Naive solution: 1 draw = 1 win and 1 loss; This is so we can calculate based on binomial rather than trinomial distribution.
		wins += draws;
		losses += draws;
